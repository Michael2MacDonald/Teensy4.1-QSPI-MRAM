...
void ResetHandler(void)
{
	...
	IOMUXC_GPR_GPR17 = (uint32_t)&_flexram_bank_config; // Linker: _flexram_bank_config = 0xAAAAAAAA | ((1 << (_itcm_block_count * 2)) - 1);
	// IOMUXC_GPR_GPR16 Bits 7-31 Vector table offset register out of reset
	// the reset value is 0x00200003 so it looks like the vector table offset is not changed but FlexRAM bank config source was changed to FLEXRAM_BANK_CFG
	// 00000000001000000000000000000011 // default 0x00200003
	// 00000000001000000000000000000111 // assigned 0x00200007
	IOMUXC_GPR_GPR16 = 0x00200007; // 0 ITCM enable initialization out of reset, 1 DTCM enable initialization out of reset, 2 FlexRAM bank config source (fuse/FLEXRAM_BANK_CFG), 3-6 reserved, 7-31 Vector table offset register out of reset
	// IOMUXC_GPR_GPR14 sets DTCM/ITCM size, ACMP internal bias current values, and ACMP sample_lv source select
	// bits 23–20 set DTCM size and 19–16 set ITCM size
	// the reset value is 0x00000000 so it looks like both DTCM and ITCM are set to 512 KB
	// 00000000_00000000_00000000_00000000 // default 0x00000000
	// 00000000_10101010_00000000_00000000 // assigned 0x00AA0000
...
#ifndef EXTMEM_CLOCK
	#define EXTMEM_CLOCK 0 // '0' for 88 MHz and '1' for 49.5 MHz
#endif
...
#define RAM1_RTE         0x66 // Reset Enable
#define RAM1_RST         0x99 // Reset
#define RAM1_RDID        0x9F // Read Device ID (Read ID)
#define RAM1_ENQPI       0x35 // Enter QPI Mode (Enter Quad Mode)
#define RAM1_RD          0xEB // Read (Fast Read Quad)
#define RAM1_WR          0x38 // Write (Quad Write)
// Chip information
#define RAM1_ID          0x5D0D // Device ID
#define RAM1_ID_MASK(id) (id & 0xFFFF) // Read only the least significant 16 bits of 'id'
 ...
 */
// Opcodes
#define RAM2_EXQPI       0xFF // Exit QPI Mode (Enable SPI)
#define RAM2_RTE         0x66 // Reset Enable (Software Reset Enable)
#define RAM2_RST         0x99 // Reset (Software Reset)
#define RAM2_RDID        0x65 // Read Any Register Address Based (To Read Chip ID)
#define RAM2_RDIDA       0x30 // Address for ID register
#define RAM2_ENQPI       0x38 // Enter QPI Mode (Enable QPI)
#define RAM2_RD          0x0B // Read (Fast Read - SDR)
#define RAM2_WR          0xDA // Write (Fast Write - SDR)
// Opcodes for setup
#define RAM2_WRAR        0x71 // Write To Register (Write Any Register - Address Based)
// #define RAM2_WRAR        0x87
#define RAM2_CR4         0x05 // Configuration Register 4 Address (Contains WREN Reset Selector)
#define RAM2_CR4V        0x05 // Value to write to CR4 to disable 'Write Enable'
#define RAM2_CR2         0x03 // Configuration Register 4 Address (Contains Read Latency Selection)
#define RAM2_CR2V        0x0C // Value to write to CR4 to set 'Read Latency' to 12
// Chip information
#define RAM2_ID          0xE6 // Chip ID
#define RAM2_ID_MASK(id) (id & 0xFF) // Read only the least significant 8 bits of 'id'
#define RAM2_ADDR        0x800000
...
FLASHMEM static uint32_t flexspi2_ram_id(uint32_t addr)
{
	FLEXSPI2_IPCR0 = addr; // Set register IPCR0 to device address
	if (addr < RAM2_ADDR) {
		FLEXSPI2_IPCR1 = FLEXSPI_IPCR1_ISEQID(3) | FLEXSPI_IPCR1_IDATSZ(4); // Set LUT index | Data Size
	} else {
		FLEXSPI2_IPCR1 = FLEXSPI_IPCR1_ISEQID(8) | FLEXSPI_IPCR1_IDATSZ(4); // Set LUT index | Data Size
	}
	FLEXSPI2_IPCMD = FLEXSPI_IPCMD_TRG; // Trigger command
	while (!(FLEXSPI2_INTR & FLEXSPI_INTR_IPCMDDONE)); // Wait until IP command is done
	...
	if (addr < RAM2_ADDR) {
		return RAM1_ID_MASK(id);
		// return id;
	} else {
		return RAM2_ID_MASK(id);
		// return id;
	}
}

// static uint32_t flexspi2_read(uint32_t cmd, uint32_t addr, uint32_t sz) {
// 	FLEXSPI2_IPCR0 = addr; // Set register IPCR0 to device address
...
// 	FLEXSPI2_IPCMD = FLEXSPI_IPCMD_TRG; // Trigger command
// 	while (!(FLEXSPI2_INTR & FLEXSPI_INTR_IPCMDDONE)); // Wait until IP command is done
// 	uint32_t val = FLEXSPI2_RFDR0; // IP RX FIFO Data Register 0 (0x402A4100)
// 	FLEXSPI2_INTR = FLEXSPI_INTR_IPCMDDONE | FLEXSPI_INTR_IPRXWA;
// 	return val;
// }

static void flexspi2_write(uint32_t cmd, uint32_t addr, uint32_t data, uint32_t sz) {
	FLEXSPI2_TFDR0 = data;
...
FLASHMEM void configure_external_ram()
{
	...
	FLEXSPI2_INTEN = 0;
	FLEXSPI2_FLSHA1CR0 = 0x2000; // 8 MBytes
	...
	FLEXSPI2_FLSHA2CR0 = 0x200; // 512 KBytes
	FLEXSPI2_FLSHA2CR1 = FLEXSPI_FLSHCR1_CSINTERVAL(2)
		| FLEXSPI_FLSHCR1_TCSH(3) | FLEXSPI_FLSHCR1_TCSS(3);
	FLEXSPI2_FLSHA2CR2 = FLEXSPI_FLSHCR2_AWRSEQID(11) | FLEXSPI_FLSHCR2_AWRSEQNUM(0) // AWRSEQID tells it what LUT entry to use to write to the ram
		| FLEXSPI_FLSHCR2_ARDSEQID(10) | FLEXSPI_FLSHCR2_ARDSEQNUM(0); // ARDSEQID tells it what LUT entry to use to read from the ram
	...
	FLEXSPI2_LUT4 = LUT0(CMD_SDR, PINS1, RAM1_RTE);
	// cmd index 2 = reset
	...
	// cmd index 8 = read ID bytes (Read any register by address)
	FLEXSPI2_LUT32 = LUT0(CMD_SDR, PINS1, RAM2_RDID) | LUT1(ADDR_SDR, PINS1, 24);
	FLEXSPI2_LUT33 = LUT0(DUMMY_SDR, PINS1, 8) | LUT1(READ_SDR, PINS1, 1); // 8 delay cycles for 1-1-1 and 2 cycles for 4-4-4
	// cmd index 9 = enter QPI mode
	FLEXSPI2_LUT36 = LUT0(CMD_SDR, PINS1, RAM2_ENQPI);
	// cmd index 10 = read QPI (Fast Read)
	FLEXSPI2_LUT40 = LUT0(CMD_SDR, PINS4, RAM2_RD) | LUT1(ADDR_SDR, PINS4, 24);
	FLEXSPI2_LUT41 = LUT0(MODE8_SDR, PINS4, 0xFF) | LUT1(DUMMY_SDR, PINS4, 12);
	FLEXSPI2_LUT42 = LUT0(READ_SDR, PINS4, 1);
	// cmd index 11 = write QPI (Fast Write)
	FLEXSPI2_LUT44 = LUT0(CMD_SDR, PINS4, RAM2_WR) | LUT1(ADDR_SDR, PINS4, 24);
	FLEXSPI2_LUT45 = LUT0(MODE8_SDR, PINS4, 0xFF) | LUT1(WRITE_SDR, PINS4, 1); // MODE8_SDR: XIP is enabled by entering byte Axh and disabled by entering byte Fxh
	// cmd index 12 = Write any register (for setting latency)
	FLEXSPI2_LUT48 = LUT0(CMD_SDR, PINS1, RAM2_WRAR) | LUT1(ADDR_SDR, PINS1, 24);
	FLEXSPI2_LUT49 = LUT0(WRITE_SDR, PINS1, 1);
	// cmd index 13 = 
	// FLEXSPI2_LUT52 = ;
	// cmd index 14 = 
	// FLEXSPI2_LUT56 = ;
	...
	flexspi2_command(0, RAM1_ADDR); // exit quad I/O mode
	flexspi2_command(1, RAM1_ADDR); // reset enable
	flexspi2_command(2, RAM1_ADDR); // reset (is this really necessary?)
	if (flexspi2_ram_id(0x00) == RAM1_ID) { // check for ram 1
		// ram 1 present
		flexspi2_command(4, RAM1_ADDR); // enter quad mode
		
		// first RAM chip is present, look for a second PSRAM chip
		flexspi2_command(7, RAM2_ADDR); // exit quad mode
		flexspi2_command(1, RAM2_ADDR); // reset enable
		flexspi2_command(2, RAM2_ADDR); // reset (is this really necessary?)
		flexspi2_write(12, RAM2_ADDR+RAM2_CR2, RAM2_CR2V, 1); // set read latency to 12
		if (flexspi2_ram_id(RAM2_ADDR+RAM2_RDIDA) == RAM2_ID) { // check for ram 2
			// ram 2 present
			flexspi2_command(9, RAM2_ADDR); // enter quad mode
			// delayMicroseconds(50);
		}
		
		// TODO: zero uninitialized EXTMEM variables
		// TODO: copy from flash to initialize EXTMEM variables
		// Teensy PSRAM chip is present
		external_psram_size = 8; // only using one memory section
		sm_set_pool(&extmem_smalloc_pool, &_extram_end, // _extram_end is the end of user varuables not the end of the section
			external_psram_size * 0x100000 - // MBytes -> Bytes
